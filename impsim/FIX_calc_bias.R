...
#biasVals <- c("value","bias", "pctBias", "covRate", "medianWidth", "RMSE", "SD")
...
print(seeds)
print(class(seeds))
cat("\n>> models from the files:", paste(names(mods)), collapse="\t")
#print(mods)
#print(str(flist))
#print(dim(flist))
#fl1 <- flist[[1]]
...
#        print(trueVals)
            ...
            for(r in params$nrun){
                trueDat <- mkResp(seed=run+seed,resp_list, mods4sim[mod], nnest, cMat, mList, beta_list, fprob, sprob, prList, debug=params$deb)
                trueFit <- summary(glm(as.formula(paste0(resp,mod)),
                           family=binomial,
                           data=sDat,
                           method=brglm2::brglmFit, 
                           control=brglmControl(maxit=iter)))
                trueVals <- exp(coef(trueFit)[vars])
                if (params$debug) cat("\n +++ true param values for this data set:\n", trueVals)
                ...
                for(v in vars){
                    ...
                    #if(params$debug) print(str(impDat))
                    #if
                    impDat[v,"cc",r,,z,resp] <- exp(impDat[v,"cc",r,,z,resp]) 
                    if(params$debug) print(str(impDat[v,,r,,z,resp]))
                    ...
                    #bias[v,,"medianWidth",z,resp] <- rowMeans(impDat[v,,,"97.5 %",z,resp] - impDat[v,,,"2.5 %",z,resp])
