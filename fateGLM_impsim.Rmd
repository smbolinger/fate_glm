...
if(params$linux==FALSE) hdir <- "C:/Users/sarah/Dropbox/Models/fate_glm/"
```
...
for(z in seq_along(mods4sim)){
  ...
  for(r in resp_list){
    ...
    biasfile <- paste0(hdir, sprintf("out/bias_vals_%s_%s.rds", r, names(mods4sim)[z]))
    # saveRDS(bias_out, sprintf("out/bias_vals_%s_%s.rds",r, names(mods4sim)[z]))
    saveRDS(bias_out, biasfile)
    biasfile1 <- paste0(hdir, sprintf("out/bias_vals_%s_%s.csv", r, names(mods4sim)[z]))
