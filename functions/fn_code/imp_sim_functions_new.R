...
# mkImpSim <- function(fullDat, ampDat, cols, resp_list, mods, vars, met, form_list, m=20, fam=binomial, regMet="brglm_fit", iter=500, debug=FALSE, xdebug=FALSE, impplot=FALSE){
mkImpSim <- function(fullDat, ampDat, prVars, resp_list, mods, vars, met, forms, m=20, fam=binomial, regMet="brglm_fit", iter=500, debug=FALSE, xdebug=FALSE, impplot=FALSE){
  ...
  if (met == "cc"){
    ...
    ampDat <- ampDat %>% select(all_of(col_list)) # need to select the cols that are relevant for mod?
    ...
    for(y in seq_along(mods)){
      # resp <- resp_list[y]
      col_list <- c(resp, prVars)
      # form_list <- form_lists[[resp]]
    ...
    for(y in seq_along(mods)){
      ...
      # metList <- na.omit(metLists[resp,,met,y, drop=F])
      # metList <- metLists[resp,,met,y, drop=F]
      resp <- resp_list[y]
      # form_list <- forms[[resp]][[y]][[met]]
      form_list <- forms[[resp]]
      ...
      # dimnames(metList)
      impCall <- case_when(met=="stratify"~ "impStrat(ampDat, met=metList,formulas=frmla, col_sel=col_list)",
                           # met=="passive" ~ "mice::mice(ampDat, method=metList, m=m, formulas=frmla, maxit=10, print=FALSE)",
                           met=="passive" ~ "mice::mice(ampDat, m=m, formulas=frmla, maxit=10, print=FALSE)",
