...
resp_list <- rep(c("is_u", "HF_mis"),3)
modList <- readLines("modList.txt")
mods4sim <- modList[c(1,8,16) ]
# mods4sim <- paste(resp_list,modList[c(1,8,16) ])
# mods4sim <- sapply(resp_list, function(x) paste(x, modList[c(1,8,16)]))
# mods4sim
...
seeds <- c(613890, 829850, 1028910, 111530, 713580)
...
#   cat("********************************************************************************************\n")
# }
# # bias_out <- parAvg(fullDat = dat5sim, impDat = imp_sim,resp = r, vars = var_list, mod = mods4sim[z], mets = met_list, biasVals = bias_names, debug = params$deb)
