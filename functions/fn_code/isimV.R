...
# misPatt <- ampMat(nd = dat4sim)
misPatt <- readRDS("misPatt.rds")
# names(misPatt)
# ampWts <- ampWt(nd = dat4sim, miss_patt_mat = misPatt)
ampWts <- readRDS("ampWts.rds")
for (seed in params$seeds){
  ...
  for(run in 1:params$nrun){
    ...
    datNA <- mkSimDat(nd = dat4sim, seeed = run+seed, wts=ampWts, mpatt = misPatt,fr= patt_freq,new_prop = 0.16, xdebug=params$xdeb, debug = params$deb, convFact = TRUE)
    datNA <- datNA$amp
