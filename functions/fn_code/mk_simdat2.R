...
mkSimDat2 <- function(seeed, nd, vars, facToNum=FALSE, method="amp", wt=TRUE, debug=FALSE, xdebug=FALSE, convFact=FALSE){
    if(method=="amp"){
        ...
        miss_pat <- list(c(0, rep(1,6)), c(1,rep(0,6)), c(rep(0,7)) )
        miss_pat
        ...
        wts <- amp_out$weights 
        wts2 <- amp_out$weights
        # wts2[1,] <- c(  0, 0, 0.8, 0.2,rep(0, 7)) # missing age only - what vars contribute
        # wts2[1,] <- c(  0, 0, 8, 2,rep(0, 7)) # missing age only - what vars contribute
        wts2[1,] <- c(  0, 0, 8, 2,rep(0, 3), rep(1,4)) # missing age only - what vars contribute
      
        wts[1,] <- c(  0, 1, 8, 2,rep(1, 7)) # missing age only - what vars contribute
        wts
        wts2
        saveRDS(wts, "ampWts.rds")
        saveRDS(wts2, "ampWts2.rds")
