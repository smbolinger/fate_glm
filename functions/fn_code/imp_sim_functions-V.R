...
mkSimDat <- function(seeed, nd, mpatt, wts, new_prop=0.16, fr=c(0.45,0.45,0.1),wt=TRUE, debug=FALSE, xdebug=FALSE, convFact=FALSE){
  ...
  #   # Easier to reference names?
    # is_miss <- names(mpatt)[!names(mpatt) %in% no_miss]
    ...
    # suppressWarnings(amp_out <- mice::ampute(dat4amp, prop = new_prop, patterns = mpatt, freq = patt_freq))
    # if(FALSE){
