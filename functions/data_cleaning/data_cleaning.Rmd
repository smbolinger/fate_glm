...
for(y in seq_along(yearList)){
  dataList[[y]] = importNest(yearnum=y, years=yearList)
}

allDat <- data.table::rbindlist(dataList, use.names=T, fill=T)
allData <- as.data.frame(allDat)
# filename21 <- 'nest_data/2021_nests_3.csv'
# filename21 <- list.files(path=importDir, pattern='2021', full.names=TRUE) |> max()
# filename20 <-  list.files(path=importDir, pattern='2020', full.names=TRUE) |> max()
# filename19 <-  list.files(path=importDir, pattern='2019', full.names=TRUE) |> max()
# 
...
# table(allData$species, useNA = "ifany")
...
rmNames <- c("pic", "ad_band", "ch_band", "pred_type", "NOTES", "found", "cam_diff", "cam_notes", "new_notes", "issue", "phot_loc", "cam_num", "k-final?", "Column1", "0", "photo") # names of columns to be removed (in quotes!)
...
newOrder <- c(names(nestdata[-colIndex]), paste0("j_",jDates))# add j to the desired col order
...
for( col in colIndex ) {
    ...
    oldCol <- nestdata[[col]]
    #nCharOld <- as.numeric(oldCol[which.max(nchar(oldCol))])
    #nCharOld <- max(nchar(oldCol), na.rm=T) 
    # oldCol <- format(oldCol, width=nCharOld)
    # oldCol
    #browser()
    newCol <- repCols[[col]]
    # nCharNew <- max(nchar(newCol), na.rm=T)
    # newCol <- format(newCol, width=nCharNew)
    ID     <- nestdata$nest
    ...
    #browser()
    #Cols   <- subset(Cols, !is.na(oldCol)) # non-empy columns
    # since I formatted it beforeehand, NA is now character
    Cols   <- subset(Cols, !is.na(Cols$oldCol)) # non-empty column
    
    #notExtr <- append(notExtr, c(col, c(which(is.na(Cols$newCol)))))
    # columns that aree empty after extraction and not before were not properly extracted
    ...
    # write.table(Cols,          file = filepath1,  # this doesn't properly justify the columns
    #             append = T,    sep="||",
    #             #append = T,    sep="\t|",
...
### Make observations into rows - CODE
```{r}
