...
# ndGLM_all <- ndGLM1 
# table(ndGLM_all$cam_fate, useNA = "ifany")
# table(ndGLM_all$nest_age, useNA = "ifany")
```
...
#### Create the dataset to be used for imputation of both nest age and true fate:
...
#### This creaes the dataset to be used for the imputation of nest age only (remove missing cam fate)
...
#### This reduces the number of fate categories for both datasets :
...
ndGLM_all <- ndGLM_all %>%
  refactor(num_fate=nfate)
levels(ndGLM_all$cam_fate)


ndGLM_all %>% sapply(function(x) sum(is.na(x)))
# var <- "nest_age"
# stillMissing <- ndGLM_all$nest[which(is.na(ndGLM_all[[var]]))]
