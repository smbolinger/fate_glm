...
setwd("C:/Users/sarah/Dropbox/Models/fate_glm/")
...
importDir <- "nest_data"
dataList <- list()
yearList <- c('2019', '2020', '2021')

for(y in seq_along(yearList)){
  dataList[[y]] = importNest(yearnum=y, yrs=yearList)
}

allDat <- data.table::rbindlist(dataList, use.names=T, fill=T)
allData <- as.data.frame(allDat)
```

```{r}
notes <- data.frame(allData$nest, allData$NOTES, allData$cam_notes, allData$new_notes )
...
vector "rmNames" below. 

```{r}

rmNames <- c("pic", "ad_band","estHD2", "ch_band", "pred_type", "NOTES", "found", "cam_diff", "cam_notes", "new_notes", "issue", "phot_loc", "cam_num", "k-final?", "Column1", "0", "photo") # names of columns to be removed (in quotes!)
...
```{r}
nestdata <- calcJDate(colIndex=colIndex, nestdata=nestdata, debug=T)
```
...
```{r include = FALSE}
unStr <- uniqueStr(colIndex, nestdata)
#stringE <- str_detect(uniqueUnique, "[0-9][:LETTER:]")
stringE <- str_detect(unStr, "[0-9][:LETTER:]")
stringNoE  <- unStr[stringE == F]
# this should match to what we end up with after extraction
write(stringNoE, sprintf("output/unique_noE%s.txt", now))
obsPerCol <- colSums(!is.na(nestdata[,colIndex]))  # Number of observations in each observation column (columns in colIndex)
totalObs  <- sum(obsPerCol)  # Total number of observations in the data before extraction
nUnique   <- length(unStr)
```
...
this also allows you to capture only the essential information.
 [see notes file for more]

```{r, include=FALSE, warning=FALSE, message=FALSE}
#\bD\b etc are not working?
strings <- c("[0-9][:LETTER:]", "[Bb][Oo][Nn]", "\\bH\\b", "[Hh]atch", "Hu", "chick behavior", "nest behavior", "wet chick", "pipping", "poops", "[Ff]ail", "\\bF\\b", "\\bW\\b", "\\bD\\b", "depredated", "coyote", "no stick", "stick missing", "not locate", "DNF", "DNL", "not f", "not [Cc]heck", "Didn't check", "not see", "not acc", "not obs", "\\bU\\b", "unk", "\\?", "empty", "no activity", "nothing", "no chicks or parents", "eaten")

strings <- paste(strings, sep = "|", collapse = "|")

nestdata <- replCols(colIndex, nestdata, strings)
```
...
```{r}
## write each observation history to file so we can look for inconsistencies
## for example, sometimes nests have multiple 0E observations after nest has reeached fate
## or, sometimes hatch date from camera is recorded in the observation history, but this
## shouldn't be counted as a field observation

## need to view them first to figure out how to remove them:
fname <- paste0("output/obs_histories_RUTH_", now, ".txt")
ndat <- nestdata |> filter(site %in% c("RUTW", "RUTE"))
sink(fname)
for(n in seq_along(ndat$nest)){
  print(obsHist(ndat[n,]), row.names=F)
}
sink()
```
...
```{r eval=FALSE, include=FALSE}
nestData1 <- read_csv('nest_data/nest_data_combined_0829_1539_.csv',
                     col_types = cols(.default="c")) # specify type - see notes
```
...
#### CODE:
```{r}
nestData1 <- as.data.frame(nestData1)
...
**Prob want to re-calculate later aftere removing excess obs**


#### CODE:
```{r, warning=FALSE}
# nestData1 <- calcObsInt(nestData1, debug=T)
# table(nestData$nest, useNA="ifany")
# no_HD <- nestData1$nest[which(is.na(nestData1$estHD) & nestData1$final_fate!="H")]
```

### Calculate initiation dates:
#### NOTES: 
...
**Use k instead of fate_date bc we want the age when fate was determined in the field (not age when fate was reached)**
#### CODE:
```{r}
nestData <- calcObsInt(nestData1, debug=T)
# table(nestData$nest, useNA="ifany")
...
table(nestData$estHD, useNA="ifany")
table(nestData$k, useNA="ifany")

nd <- nestData %>% mutate(init=init(estHD, species, nest=nest, debug=T))
...
```{r}
nestData <- nestData %>% filter(!is.na(status))
...
fate_NA <- unique(nestData$nest[which(is.na(nestData$final_fate))])  # which nests?
nestData$nest[which(nestData$camera == T & is.na(nestData$final_fate))] 

nestData <- nestData[!is.na(nestData$final_fate),] # REMOVE NAs

camNests <- unique(nestData$nest[which(nestData$camera == T)]) # cam nest IDs
length(camNests) # how many nests with cameras, according to R?
```
...
length(camNests2) # how many camera nests once sites/species/years excluded?
```
...
filename21 <- 'nest_data/2021_nests_.csv'
filename20 <- 'nest_data/2020_nests_.csv'
filename19 <- 'nest_data/2019_nests_.csv'
...
strings <- c("[0-9][:LETTER:]", "[Bb][Oo][Nn]", "\\bH\\b", "[Hh]atch", "Hu", "chick behavior", "nest behavior", "wet chick", "pipping", "poops", "[Ff]ail", "\\bF\\b", "\\bW\\b", "\\bD\\b", "D", "depredated", "coyote", "no stick", "stick missing", "not locate", "DNF", "DNL", "not f", "not [Cc]heck", "Didn't check", "not see", "not acc", "not obs", "\\bU\\b", "unk", "\\?", "empty", "no activity", "nothing", "no chicks or parents")
...
```{r}
# table(nestData1$final_fate) # these aren't right - still pivoted to long
# table(nestData1$cam_fate)
# nestData1 |> group_by(nest) |> tbl_summary(by=final_fate)

# nestData1 |> group_by(nest) |> select(final_fate) |> summarize(nNest = n())
# ^^this gives the number of observations of each nest^^
# 
nestData1 |> group_by(nest) |> select(final_fate) |> mutate(nNest = )
```
...
# 
>>>>>>> 725f76eaf87ef2da636ba9b4a8c87c9bf9bebf77
