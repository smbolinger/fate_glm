...
cat(">> columns selected for imputation:", col_sel)
# do 1 response at a time, since they are correlated
# col_sel
# met     <- "pmm"
...
# impInt <- impStrat(datNameImp, met=impMet, col_sel, debug=TRUE)
impInt <- imputeDat(datNameImp, met=impMet, cols=col_sel, seed = 61389, m = 30, complete = FALSE, debug=TRUE)
impInt$loggedEvents

# impMets <- impInt$method
# impMets
# impMets[2] <- ""
```
...
# sim_dat <- mkSimDat(nd = dat4imp, method = "amp", wt = TRUE, debug = TRUE, convFact = TRUE)
...
imp_sim <- runSim(datNA = sim_dat$amp,col_sel = col_sel,mets = met_list, resp = resp, vars = var_list, mod = mod, nruns=nrun, debug = FALSE) # don't want to set seed
